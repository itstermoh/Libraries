...
for _,v in pairs(game.CoreGui:GetChildren()) do
    if v.Name == "ezDoors" then
        v:Destroy()
    end
end

local dark_UI = Instance.new("ScreenGui")
dark_UI.Name = "ezDoors"
...
function Library:Create(table)
    ...
    function tabHandler:Tab(name)
        ...
        main1.MouseButton1Click:Connect(function()
            for _,v in pairs(game.CoreGui:FindFirstChild('ezDoors').main:GetChildren()) do
                if v.Name == "container" then
                    v.Visible = false
                end
            end
            for _,v in pairs(game.CoreGui['ezDoors'].main.tabContainer:GetChildren()) do
        ...
        function ElementHandler:Toggle(text, callback)
            ...
            --// fixed goofy ahh toggle not working
    ...
    --// first table visible because yes
    game.CoreGui['ezDoors'].main.tabContainer.ChildAdded:Connect(function()
        game.CoreGui['ezDoors'].main:FindFirstChild('container').Visible = true
        pcall(function()
            repeat wait() until game.CoreGui['ezDoors'].main:FindFirstChild('tabContainer'):FindFirstChildWhichIsA('TextButton')
        end)
        game:GetService('TweenService'):Create(game.CoreGui['ezDoors'].main:FindFirstChild('tabContainer'):FindFirstChildWhichIsA('TextButton'), TweenInfo.new(0.3), {TextTransparency = 0}):Play()
    end)
